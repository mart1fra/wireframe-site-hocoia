        ...
        >
          {pourQui.cards.map((card) => (
            ...
            >
              ...
              <p className="text-gray-500 text-xs italic mt-4">{card.casType}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          {couverture.regions.map((region) => {
            const actif = region.statut === "actif";
            return (
              <motion.div
                key={region.id}
                variants={itemV}
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${
                      actif ? "bg-gray-900" : "bg-gray-400"
                    }`}
                  />
                  <span
                    className={`text-xs font-semibold ${
                      actif ? "text-gray-700" : "text-gray-400"
                    }`}
                  >
                    {region.label}
                  </span>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
        <FadeIn>
          ...
          <h2 className="font-display font-bold text-3xl sm:text-4xl text-gray-900 leading-tight mb-5">
            Parlons de votre projet de dépistage
          </h2>
        ...
        <FadeIn delay={0.1}>
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
            ...
            <div className="space-y-3">
              ...
              <input disabled placeholder={hero.form.communePlaceholder} className="w-full bg-white border border-gray-200 text-gray-400 placeholder-gray-400 rounded-lg px-3 py-2.5 text-sm cursor-not-allowed" />
              <select disabled className="w-full bg-white border border-gray-200 text-gray-400 rounded-lg px-3 py-2.5 text-sm cursor-not-allowed appearance-none">
                {hero.form.nombreOptions.map((o) => <option key={o}>{o}</option>)}
              </select>
...
export default function MammoBus() {
  return (
    <>
      <HeroSection />
      <LogosSection />
